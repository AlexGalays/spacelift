...
type UnionResult<T extends UnionDescription> = {
  T: Union<T>
  is: <NAME extends keyof T>(name: NAME) => <U extends Union<T>>(other: U) => other is (ReturnType<T[NAME]> & { type: NAME })
...
type Factory<F extends (...args: any[]) => any, TYPE> =
  (...args: Arguments<F>) => F extends (...args: any[]) => infer R
    ? { [K in keyof R | 'type']: K extends 'type' ? TYPE : R[K & keyof R] }
    : (...args: any[]) => any

type Union<T extends UnionDescription> = { [K in keyof T]: { [K2 in keyof ReturnType<T[K]> | 'type']: K2 extends 'type' ? K : ReturnType<T[K]>[K2] } }[keyof T]
